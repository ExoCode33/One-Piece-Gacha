...
// ENHANCED Hunt Again function with FULL rainbow double progression
async function handleHuntAgain(interaction) {
    try {
        ...
        // PHASE 1: Main rainbow animation phase (18 frames)
        for (let frame = 0; frame < 18; frame++) {
            await updateAnimationFrameButton(interaction, frame, targetFruit, performanceMetrics);
            await sleep(1000);
        }
        
        // PHASE 2: Progression crystallization phase (12 frames)  
        ...
        // PHASE 3: Transition to rarity phase (10 frames)
        ...
        // PHASE 4: Reveal information gradually
...
// PHASE 1: Rainbow animation frames with double bars and indicators
async function updateAnimationFrameButton(interaction, frame, targetFruit, metrics) {
    try {
        metrics.frameAttempts++;
        
        // Calculate rainbow positions for DOUBLE 20-square bars
        const barLength = 20;
        const positions = [];
        ...
        // Calculate embed color (rainbow cycling)
        let embedColor;
        if (frame < 16) {
        ...
        // Get animated particles
        const particles = generateParticles();
        
        // Dynamic animation text progression
        const dynamicTexts = [
        ...
        // Create full animation content with DOUBLE bars
...
// PHASE 2: Progression crystallization frames
...
// PHASE 3: Transition to final rarity reveal
async function updateTransitionFrameButton(interaction, transFrame, targetFruit, metrics) {
    try {
        ...
        // Create expanding rarity color from center
        for (let i = 0; i < barLength; i++) {
...
// PHASE 4: Gradual information reveal with full details
async function revealInformationGraduallyButton(interaction, targetFruit, elementName, userLevel) {
    try {
        ...
        // Final result with action buttons
        ...
        console.error('Reveal information error:', error);
    }
}
