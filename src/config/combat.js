...
class CombatSystem {
    ...
    // Calculate total HP based on CP (more CP = more HP)
    calculateTotalHP(totalCP) {
        const baseHP = 100;
        const hpBonus = Math.floor(totalCP / 200); // +1 HP per 200 CP
        return Math.min(baseHP + hpBonus, 300); // Cap at 300 HP
    }
    ...
    // Calculate balanced damage (reduced to ensure 3 turns)
    calculateDamage(attackerCP, attackerFruit, defenderFruit, defenderHP) {
        // Reduced base damage to ensure longer battles
        const baseDamage = attackerCP * 0.08; // Reduced from 0.25 to 0.08
        ...
        const critChance = this.rarityCritChances[attackerFruit.rarity] || 0.10;
        const isCritical = Math.random() < critChance;
        const critMultiplier = isCritical ? 1.4 : 1.0; // Reduced crit multiplier
        
        // Add some randomness (80% to 120% of calculated damage)
        const randomMultiplier = 0.8 + (Math.random() * 0.4);
        
        const finalDamage = Math.floor(baseDamage * elementalMultiplier * critMultiplier * randomMultiplier);
        
        // Ensure damage is reasonable (5-25% of max HP per hit)
        const maxDamage = Math.floor(defenderHP * 0.25);
        const minDamage = Math.floor(defenderHP * 0.05);
